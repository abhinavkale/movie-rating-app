...
const API_URL =
  "https://api.themoviedb.org/3/discover/movie?sort_by=popularity.desc&api_key=4a5a7c611d15c94352226e44804eb55c&page=";
const IMG_PATH = "https://image.tmdb.org/t/p/w1280";
const SEARCH_API =
  "https://api.themoviedb.org/3/search/movie?&api_key=4a5a7c611d15c94352226e44804eb55c&query=";
...
function renderPageNumbers() {
  ...
  if (currPageNum != 1) {
    firstPageToShow = Math.ceil(
      currPageNum - PAGES_TO_SHOW / 2 > 1 ? currPageNum - PAGES_TO_SHOW / 2 : 1
    );
    lastPageToShow =
      Math.floor(currPageNum + PAGES_TO_SHOW / 2) < PAGES_TO_SHOW
        ? PAGES_TO_SHOW
        : Math.floor(currPageNum + PAGES_TO_SHOW / 2);
  }
...
function _paginate(goTo) {
  pageNumberEl.childNodes.forEach((child) => {
    if (child.classList.contains("current-page")) {
      ...
      currPageNum =
        goTo == "next" ? currPage + 1 : currPage == 1 ? 1 : currPage - 1;
      if (!searching) {
        getMovies(API_URL + currPageNum);
      } else {
